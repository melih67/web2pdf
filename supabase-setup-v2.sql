...
-- 1. Drop existing policies and table to ensure a clean slate.
DROP POLICY IF EXISTS "Users can delete their own exports" ON public.pdf_exports;
DROP POLICY IF EXISTS "Users can insert their own exports" ON public.pdf_exports;
DROP POLICY IF EXISTS "Users can view their own exports" ON public.pdf_exports;
DROP TABLE IF EXISTS public.pdf_exports CASCADE;

-- 2. Create the pdf_exports table for the history
...
-- 5. Create security policies for the pdf_exports table
...
-- 6. Reminder for Storage Policies
...
-- 7. Utility function to clean up old exports (optional)
...
-- 8. Function to get user statistics (optional)
